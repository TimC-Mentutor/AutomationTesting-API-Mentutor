...
#  Negative
  Scenario Outline:Put update task with valid id and invalid due date
    ...
    Examples:
      ...
      | 6  | sosial | tugas       | 2023-juli-50 | MentorUpdateTask.docx | MentorUpdateTaskPetapng.png |
#  Negative
  Scenario:Put update task with invalid id and invalid body
    ...
    And Validate JSON Schema "MentorPutUpdateTaskInvalidIdJsonSchema.json"
#  Negative
  Scenario:Put update task with valid id and  past due date
    Given change id "satu" and body to title "materi" description "baba 1" due date "2023-1-20" attachment "MentorUpdateTask.docx" images "MentorUpdateTaskPetapng.png"
    When Send request mentor update task
    Then response code mentor should be 400
    And Responses body message error should be "Invalid Input From Client"
    And Validate JSON Schema "MentorPutUpdateTaskInvalidIdJsonSchema.json"
...
#  Negative
  Scenario:Post task without input due date
    Given change body to title "sosial" description "tugas" due date "" file "MentorPostTask.docx" images "MentorPostTaskPetapng.png"
    When Send request mentor post task without input due date
    Then response code mentor should be 400
    And Responses body message error should be "Invalid Input From Client"
    And Validate JSON Schema "MentorPostTaskInvalidBodyJsonSchema.json"
  #  Negative
  Scenario:Post task without input images
    Given change body to title "sosial" description "tugas" due date "2023-12-12" file "MentorPostTask.docx" images ""
...
#  Positive
  ...
  Scenario:Post with using alphabet on id and valid comment
    ...
    And Validate JSON Schema "MentorPostAddCommentDoesn'tExistIdWithoutCommentBodyJsonSchema.json"
#  Positive
  Scenario:Post with valid id and using integer on comment
    Given Post with id 3 and valid caption 123456
