...
public class AdminStepDef {
    ...
    //POST RegisterUser negative-case
    @Given("Post register new user with blank request body {string}")
    public void postRegisterNewUserWithBlankRequestBody(String jsonFile) {
        File json = new File(ConstantAdmin.REGISTER_REQ_BODY + jsonFile);
        adminMentutorAPI.postRegisterNewUser(json);
    }
    //POST RegisterUser negative-case
    ...
    @Given("Put update with valid id {int} and body to name {string} " +
    ...
    (int id_user, String name, String email, String password, String images, int id_class) {
        System.out.println(ConstantAdmin.IMAGES);
        File imageFile = new File(ConstantAdmin.IMAGES+images);
        adminMentutorAPI.putUpdateUser(id_user,name,email,password,imageFile,id_class);
